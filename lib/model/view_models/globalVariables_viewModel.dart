...
import 'package:wajah_id/model/Product.dart';
import 'package:wajah_id/model/cartItem.dart';
...
class globalVars with ChangeNotifier {
  ...
  final DocumentReference SkinProductInformation =
      FirebaseFirestore.instance.collection('Products').doc('SkinProduct');
  ...
  List<String> _categories = ['Skin Clinic','Skin Products', 'Special Offer','Doctor'];
 ...
 Map<String, List<Product>> _AllProds = {
  ...
  Future getAllCategories() async {
    DocumentSnapshot doc = await SkinProductInformation.get();
    _categories = List<String>.from(doc.get('Categories'));
    print(_categories);
  }

  Future getAllProds() async {
    //skin products wajah id
    _AllProds.clear();
    await getAllCategories();
    for (int i = 0; i < _categories.length; i++) {
      QuerySnapshot qSnapshot =
          await SkinProductInformation.collection(_categories[i]).get();
  ...
  Map<String, List<Product>> get AllCategory => AllCategory;
